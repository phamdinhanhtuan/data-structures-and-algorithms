...
void theValueOFK (node *head) {
	int pos , tmp1 , index = 0 ; 
	do {
		printf ("\nEnter the pos need to print value : ") ;
		tmp1 = scanf ("%d",&pos) ;
	...
	if (tmp == NULL) {
 	...
 	printf ("The data of element at position %d is : " , pos) ;
 	printItem (tmp -> data) ;
}

node* findElementMax (node *head) {	
	node *firstMax = head -> next ; 
	node *tmp = head -> next ; 
...
int main () {
	...
	printf ("\nNumber of node in this linked list : %d" ,countNode(head)) ;
